...
dotnet ef database drop -p Infrastructure -s API -c ElixirHandShopContext
dotnet ef migrations remove -p Infrastructure -s API -c ElixirHandShopContext 

To add new migration use the following command
dotnet-ef migrations add InitialCreate -p Infrastructure -s API -o Data/Migrations
dotnet-ef migrations add ProbrtypeMigration -p Infrastructure -s API -o Data/Migrations -c Infrastructure.Data.ElixirHandShopContext
...
dotnet ef migrations add OrderedEntityAdded -p Infrastructure -s API -c ElixirHandShopContext 
...
Install Redis in via Docker
Used the compose file in solution
...
Container for backend stop but front end still running
docker kill $(docker ps -f "Name=redis" -q)

Stop and Remove all docker containers with a single command
